...
def desc_per(request):
    ...
    if user_id:
        ...
        per = []
    has_role_per = {item['permissions']:None for item in per}
    ...
    if role_obj:
        ...
        has_permission_per = {item.id:None for item in per}
    elif user_obj:
        per = user_obj.roles.filter(permissions__id__isnull=False).values('id','permissions').distinct()
        has_permission_per = {item['permissions']:None for item in per}
    else:
        has_permission_per = {}
